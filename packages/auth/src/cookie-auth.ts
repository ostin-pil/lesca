...
import { AuthError, ConfigError } from '@lesca/error'
...
export class CookieFileAuth implements AuthStrategy {
  ...
  async authenticate(): Promise<AuthCredentials> {
    if (!this.cookiePath) {
      throw new AuthError('AUTH_INVALID_CREDENTIALS', 'Cookie file path not provided')
    }

    await this.load(this.cookiePath)

    if (!this.credentials) {
      throw new AuthError('AUTH_INVALID_CREDENTIALS', 'Failed to load credentials from cookie file')
    }
  ...
  async save(path: string): Promise<void> {
    if (!this.credentials) {
      throw new AuthError('AUTH_INVALID_CREDENTIALS', 'No credentials to save')
    }
  ...
  async load(path: string): Promise<void> {
    try {
      ...
      if (!isValid) {
        throw new AuthError('AUTH_SESSION_EXPIRED', 'Loaded credentials are invalid or expired')
      }
    } catch (error) {
...
export function exportCookies(
  ...
  throw new ConfigError('CONFIG_INVALID_VALUE', `Unsupported export format: ${String(format)}`, {
